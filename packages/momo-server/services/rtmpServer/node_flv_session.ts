...
import type { RtmpAuthConfig } from '../../types/runtime-config';
...
class NodeFlvSession {
  ...
  constructor(server: FlvServer, req: FlvRequest, res: FlvResponse) {
    this.server = server;
    this.req = req;
    this.res = res;
  ...
  stop(): void {
    if (this.isStarting) {
      ...
      if (publisherId != null) {
        (context.sessions.get(publisherId) as PublisherSession).players.delete(this.id);
  ...
  onStartPlay(): void {
    let publisherId = context.publishers.get(this.playStreamPath);
    let publisher = context.sessions.get(publisherId) as PublisherSession;
  ...
  static createFlvTag(packet: FlvPacketType): Buffer {
