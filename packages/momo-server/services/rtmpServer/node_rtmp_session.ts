...
type AMFMessage = Record<string, unknown>;

var RtmpPacket = {
  create: (fmt = 0, cid = 0): RtmpPacketType => {
    return {
      header: {
        fmt: fmt,
        cid: cid,
...
class NodeRtmpSession {
  [key: string]: unknown;
  server: { rtmpServer?: RtmpServerConfig };
  socket: Socket;
  id: string;
  ip: string;
  TAG: string;
  ...
  parserPacket: RtmpPacketType;
  inPackets: Map<number, RtmpPacketType>;
  inChunkSize: number;
  outChunkSize: number;
  pingTime: number;
  pingTimeout: number;
  ...
  connectCmdObj: InvokeMessage['cmdObj'] | null;
  objectEncoding: number;
  startTime: Date;
  startTimestamp: number;

  constructor(socket: Socket, server: unknown) {
    const serverRef = server as { rtmpServer?: RtmpServerConfig };
    const rtmpServerConf: Partial<RtmpServerConfig> = serverRef.rtmpServer || {};
  ...
  sendInvokeMessage(id: number, opt: AMFMessage): void {
    let packet = RtmpPacket.create();
    packet.header.fmt = RTMP_CHUNK_TYPE_0;
  ...
  sendDataMessage(opt: AMFMessage, id: number): void {
    let packet = RtmpPacket.create();
    packet.header.fmt = RTMP_CHUNK_TYPE_0;
    packet.header.cid = RTMP_CHANNEL_DATA;
    packet.header.type = RTMP_TYPE_DATA;
  ...
  onConnect(invokeMessage: InvokeMessage): void {
  ...
  onCreateStream(invokeMessage: InvokeMessage): void {
    this.respondCreateStream(invokeMessage.transId);
  }

  onPublish(invokeMessage: InvokeMessage): void {
  ...
  onPlay(invokeMessage: InvokeMessage): void {
    if (typeof invokeMessage.streamName !== 'string') {
      return;
    }
  ...
  onPause(invokeMessage: InvokeMessage): void {
