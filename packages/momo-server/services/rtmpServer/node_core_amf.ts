 ...
 */
type AMFAnyResult = { len: number; value: unknown };
type AMFObject = Record<string, unknown>;
const amf3dRules = {
...
interface StrictArrayLike extends Array<unknown> {
  sarray?: boolean;
}
...
function amfType(o) {
  ...
  if (jsType == 'object') {
    if (o instanceof Array) {
      if ((o as StrictArrayLike).sarray) return 'sarray';
      return 'array';
    }
    return 'object';
  }
 ...
 */
function amf3decString(buf): AMFAnyResult {
 ...
 * @returns {{len: *, value: (*|String)}}
 */
function amf3decXmlDoc(buf): AMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
  let s = sLenInfo.value & 1;
  let sLen = sLenInfo.value >> 1; // The real length without the lowest bit
  if (s)
 ...
 * @returns {{len: *, value: (*|String)}}
 */
function amf3decXml(buf): AMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
  let s = sLenInfo.value & 1;
  let sLen = sLenInfo.value >> 1; // The real length without the lowest bit
  if (s)
 ...
 */
function amf3decByteArray(buf): AMFAnyResult {
  const sLenInfo = amf3decUI29(buf);
 ...
 * @returns {{len: *, value: *}}
 */
function amf3decArray(buf): AMFAnyResult {
 ...
 * @param buf
 */
function amf3decObject(buf): AMFAnyResult {
 ...
 */
function amf0decObject(buf): AMFAnyResult {
...
function amfXDecode(rules, buffer) {
  ...
  for (let i = 0; i < buffer.length; ) {
    res = amfXDecodeOne(rules, buffer.slice(i)) as AMFAnyResult;
    i += res.len;
 ...
 */
function decodeAmf0Data(dbuf: Buffer): AMFObject {
  let buffer = dbuf;
 ...
 * @returns {{cmd: (*|string|String|*), value: *}}
 */
function decodeAMF0Cmd(dbuf: Buffer): AMFObject {
  let buffer = dbuf;
  let resp: Record<string, unknown> = {};
 ...
 * @returns {*}
 */
function encodeAMF0Cmd(opt: AMFObject): Buffer {
...
function encodeAMF0Data(opt: AMFObject): Buffer {
  let data = amf0EncodeOne(opt.cmd);
  const cmd = String(opt.cmd);
 ...
 * @returns {{}}
 */
function decodeAMF3Cmd(dbuf: Buffer): AMFObject {
  let buffer = dbuf;
  let resp: Record<string, unknown> = {};
 ...
 * @returns {*}
 */
function encodeAMF3Cmd(opt: AMFObject): Buffer {
  let data = amf0EncodeOne(opt.cmd);
  const cmd = String(opt.cmd);
