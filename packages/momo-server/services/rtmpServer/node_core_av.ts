...
interface HEVCSpsInfo {
  ...
  profile_tier_level?: HEVCPtlInfo;
...
function getSampleRate(bitop: Bitop, info: AACInfo): number {
  info.sampling_index = bitop.read(4);
  return info.sampling_index == 0x0f ? bitop.read(24) : AAC_SAMPLE_RATE[info.sampling_index];
}

function readAACSpecificConfig(aacSequenceHeader: Buffer): AACInfo {
  let info = {} as AACInfo;
...
function getAACProfileName(info: AACInfo) {
...
function readH264SpecificConfig(avcSequenceHeader: Buffer): AVCInfo {
  let info = {} as AVCInfo;
  let profile_idc,
    width,
    height,
...
function HEVCParsePtl(bitop: Bitop, hevc: HEVCInfo, max_sub_layers_minus1: number): HEVCPtlInfo {
  let general_ptl = {} as HEVCPtlInfo;
...
function HEVCParseSPS(SPS: Buffer, hevc: HEVCInfo): HEVCSpsInfo {
  let psps = {} as HEVCSpsInfo;
...
function readHEVCSpecificConfig(hevcSequenceHeader: Buffer): AVCInfo {
  let info = {} as AVCInfo;
  info.width = 0;
  info.height = 0;
  info.profile = 0;
...
function readAVCSpecificConfig(avcSequenceHeader: Buffer): AVCInfo | undefined {
...
function getAVCProfileName(info: AVCInfo) {
  switch (info.profile) {
    case 1:
      return 'Main';
    case 2:
